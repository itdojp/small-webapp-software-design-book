...
**分類: 実行可能例** — `examples/typescript/src/chapter-04/domain/task.ts` と同期し、CI で型検査します。
<!-- code-example: executable path=examples/typescript/src/chapter-04/domain/task.ts -->
...
**分類: 実行可能例** — `examples/typescript/src/chapter-04/contracts/assignment.ts` と同期し、CI で型検査します。
<!-- code-example: executable path=examples/typescript/src/chapter-04/contracts/assignment.ts -->
...
- 認証/session middleware: usecase実行前にactorを確定し、sessionなし/無効/期限切れ/失効済みを`unauthorized/401`へ写像する。認証済みの権限不足`forbidden/403`と分ける

例（認証成立後にusecaseが返すエラーをコード化する最小形式）:

**分類: 実行可能例** — `examples/typescript/src/chapter-04/contracts/errors.ts` と同期し、CI で型検査します。
<!-- code-example: executable path=examples/typescript/src/chapter-04/contracts/errors.ts -->
...
- 認証・セッション契約（任意）: [Appendix B: テンプレ集（B-17）]({{ '/appendix/B-templates/' | relative_url }}) / [Appendix D: 記入例（D-25）]({{ '/appendix/D-samples/' | relative_url }})
...
### 最小例（設計メモ/責務分割/ディレクトリ構成）

初学者が迷いやすい点（何をどこに置くか、何を成果物として残すか）を、ランニング例に寄せた最小例で示します。フレームワークやチーム規約に合わせて調整してください。

責務分割（最小）:

| 区分 | 主な責務 | 入出力の例 | 主なテスト |
| --- | --- | --- | --- |
| domain | ルール（期限判定、状態遷移、入力バリデーション） | `canTransition(from,to)` | 単体 |
| usecases | ユースケースの制御（永続化、通知要求の発行） | `assignTask(deps,input)` | 単体（port をスタブ化） |
| adapters | I/O（HTTP/DB/通知）、形式変換、エラー整形 | handler/repository/mailer | 統合（境界の契約） |
