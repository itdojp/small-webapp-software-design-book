...
- 距離の痛み（主に D）:
  ...
  - 外部I/F（通知、決済、認証など）で失敗・遅延・仕様差分が顕在化する
...
| 外部I/Fの失敗が業務影響を持つ | D | 失敗時の期待（業務継続/再送/検知）を受け入れ条件に入れる | adapter 強化（タイムアウト/リトライ）、アウトボックス等 |
...
### 2) 権限が増殖する（ロール拡張/例外追加/UI と API の不整合）
