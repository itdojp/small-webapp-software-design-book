...
- Then:
  - タスクの担当者が更新される
  - 担当者への通知要求が作成される（例: outbox への記録。送信は非同期で実行してよい）
...
- **認証（Authentication）**: 主体をどう確認し、未認証・無効・期限切れ・失効済みの認証情報をどう扱うか（`unauthorized/401`）。方式とセッションの生存期間は [Appendix B（B-17）]({{ '/appendix/B-templates/' | relative_url }}) で固定する
- **認可（Authorization）**: 認証済みの主体が対象へ何を実行できるか/権限不足時の応答（`forbidden/403`、監査ログ）。認証と混同せず、[Appendix B（B-13）]({{ '/appendix/B-templates/' | relative_url }}) の認可ルール表へ接続する
- **対象の存在（Not Found）**: 存在しないID参照時の応答（例: `404`、表示文言、ログ方針）
- **入力検証（Validation）**: 不正入力の扱い（例: `400`、エラー形式、UI での表示）
- **状態/業務ルール違反（Rule Violation）**: 不正な状態遷移や制約違反の扱い（例: `409`、理由コード。HTTP は粗い分類とし `code` を主にする）
...
- 可用性: 営業時間（例: 平日 09:00-18:00 JST）の主要導線 API 成功率が 99.9% 以上（測定: 5xx/タイムアウト率をメトリクス集計）
- 性能: 主要導線 API の応答時間 p95 が 1 秒以内（測定: APM/アクセスログから集計）
- セキュリティ: 認証と認可は API 側で強制し、cookie session を選ぶ場合は状態変更リクエストに CSRF 対策を適用する / セッションID・トークン・個人情報はログに出さない
- 監査/運用: 「誰が/いつ/何をしたか」が追える（測定: 監査ログの必須フィールドを固定し、保持期間（例: 180日）を合意する）
...
- [ ] 各要件に紐づく受け入れ条件（仕様）が定義され、検証可能になっている
