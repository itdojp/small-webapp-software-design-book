...
| B-4 | D-4 | S/D/V で依存の痛みを半定量化する |
| B-5 | D-5 | 設計（内部構造）を最小でアウトライン化する |
...
| B-4 | D-12 | S/D/V で依存の痛みを半定量化する |
| B-5 | D-13 | 設計（内部構造）を最小でアウトライン化する |
...
- API（例）:
  ...
  - 成功時は `200` を返す
- 外部連携（通知）:
  - 割り当て時に担当者への通知要求を作成する（例: outbox に記録し、送信はワーカーが非同期に実行する）
  - 送信処理は外部I/Fの失敗/遅延が起き得るため、失敗時の期待（業務継続/検知/再送）を定義する
...
- Given: 管理者がログインしており、タスクが `todo` 状態で存在する
  ...
  - Then:
    ...
    - 画面に「割り当て完了」が表示される
    - 担当者への通知要求（例: outbox レコード）が作成される（送信は非同期で遅延し得る）
...
- Given: 割り当てにより通知要求が作成されている
  - When: 通知送信ワーカーが送信を試行し、外部I/Fが一時的に失敗する（例: タイムアウト）
  - Then:
    - 通知要求は失われず、再送起点（outbox 等）に残る
...
## D-5. 設計（Design / Structure）アウトラインの記入例（B-5）
...
- adapters が port を実装し、外部I/F（DB/メール等）を扱う
...
- 問題: メール送信は外部I/F であり、失敗/遅延が発生する
- 制約: 割り当ては業務上の確定であり、通知失敗で割り当て自体を失敗させたくない

#### Decision

- 採用する決定: 割り当ての永続化は成功させ、通知失敗は「検知 + 再送方針」で扱う
- 理由: D が大きい外部I/F である一方、割り当ての S は強い（同時変更が必須）。境界（adapter）で失敗を吸収し、ユースケースは業務確定を優先する
...
- 外部連携（通知）:
  - 期限超過時に担当者への通知要求を作成する（例: outbox に記録し、送信はワーカーが非同期に実行する）
...
- Given: タスクが期限超過になった（`dueAt < now`）
  ...
  - Then:
    - 担当者への期限超過通知要求（例: outbox レコード）が作成される（送信は非同期で遅延し得る）
    - 通知要求と送信結果（成功/失敗）が追跡できる
...
## D-12. S/D/V 採点（依存関係トップ）の記入例（B-4）
...
## D-13. 設計（Design / Structure）アウトラインの記入例（B-5）
...
- 通知（メール）送信失敗は、割り当ての合否と混同しない（ケース1 D-3/D-4）。API の失敗として返すのではなく、運用で検知できる記録（ログ/メトリクス/監査）に落とす。
...
| --- | --- | --- | --- | --- | --- | --- |
| R-1 | 管理者が担当者を割り当てできる | D-3「受け入れ条件」/「観測点」 | D-5（`assignTask`） | D-8（統合/E2E） | 完了 | 主要導線 |
| R-2 | 管理者のみ割り当て可能（認可） | D-3「例外系: 認可失敗」 | D-5（domain ルール）, D-18（`forbidden`） | D-8（単体/統合） | 完了 | 権限逸脱は許容しない |
| R-3 | 割り当ての監査記録が残る | D-3「観測点: 監査/ログ」 | D-5（`AssignmentAudit`）, D-7（監査の扱い） | D-8（統合） | 完了 | 保存期間は運用要件次第 |
| R-4 | 割り当て時に通知できる | D-3「外部連携（通知）」 | D-5（mailer adapter） | D-8（統合: stub/spy） | 完了 | 到達性は外部要因 |
...
| 要件ID | Shall（要約） | 仕様（受け入れ条件/観測点） | 設計（境界/ADR/エラーcode等） | テスト（単体/統合/E2E） | 状態 | 備考 |
| --- | --- | --- | --- | --- | --- | --- |
| DUE-1 | 期限ステータスを判定できる | D-11「受け入れ条件」/「時刻/タイムゾーン」 | D-13（期限判定を domain へ） | D-16（単体） | 完了 | 時刻は注入して固定 |
| DUE-2 | 期限ステータスを表示できる | D-11「外部I/F: UI」/「観測点」 | D-13（DTO に必要情報のみ） | D-16（統合/E2E） | 完了 | 表示要件は変動しやすい |
| DUE-3 | 期限超過時に通知できる | D-11「外部連携（通知）」 | D-13（通知 I/F を境界化） | D-16（統合） | 完了 | |
| DUE-4 | 期限超過通知が重複しない | D-11「冪等性（二重実行）」 | D-13（冪等キー/記録） | D-16（単体/統合） | 完了 | 二重実行を前提にする |
| DUE-5 | 通知結果を運用で検知できる | D-11「観測点」 | D-13（ログ/メトリクス） | D-16（統合） | 完了 | |
...
| AUTHZ-2 | update_status | task | `assigneeId == actorId` | forbidden/403 | actor/taskId/code/相関ID | 統合/単体 | 状態遷移ルールは D-3/D-5 を参照 |
...
- 方針（例）: 楽観ロック
  ...
  - 競合時は `conflict/409` を返す（後勝ちにする場合も、方針を曖昧にしない）
- 再試行（例）:
  - API: クライアント側で再試行するかは、業務影響と UX を見て決める（上限/間隔を固定）
...
- 手動再送: Yes
  ...
  - 権限: admin のみ
  - 手順（例）: `failed` を `pending` に戻し再送（相関IDで追跡）
- 冪等性/重複抑止（例）:
