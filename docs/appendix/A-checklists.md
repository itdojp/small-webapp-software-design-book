...
- セキュリティ:
  - [ ] 認証（主体確認）と認可（操作可否）が分離され、未認証/無効/期限切れ/失効済みは `401`、認証済み権限不足は `403` で一貫している
  - [ ] cookie session / bearer token の方式選択と、保存場所・送信方法・主要脅威・適用条件が記録されている
  - [ ] cookie方式では `Secure` / `HttpOnly` / `SameSite` を明示し、状態変更へ CSRF token と Origin/Referer 等の検証を適用している
  - [ ] idle timeout / absolute timeout、login・権限変更時のrotation、logout時のserver-side invalidation、失効条件が固定されている
  - [ ] 認証/認可がサーバ側で強制されている（UI の表示制御に依存しない）
  - [ ] 入力検証（形式/長さ/必須）が境界（HTTP）で一貫している
  - [ ] 機微情報（セッションID、トークン、個人情報、パスワード相当）をログに出さない
