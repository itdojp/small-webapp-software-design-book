...
- 優先度（Must/Should/Could）は **MoSCoW の優先度**（スコープ管理のための合意）として扱います
- RFC 2119 の MUST/SHOULD 等（プロトコル仕様の規範語）とは別物です
...
- HTTP は粗い分類とし、**最終的な意味は `code` を主**にします（同じ `409` でも `code` で区別し得る）
- `422` を使う流派もありますが、本書のテンプレでは `invalid` を `400` に寄せています（必要なら運用で置き換えてください）
